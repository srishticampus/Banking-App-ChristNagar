...
function ClerkViewonlineTransaction() {
  ...
  const ApplicationData = async () => {
    try {
      const response = await axiosInstance.post("/viewNonVerifiedTransactions");
  ...
  const VerifiedApplicationData = async () => {
    try {
      const response = await axiosInstance.post("/viewVerifiedTransactions");
...
export default ClerkViewonlineTransaction;
